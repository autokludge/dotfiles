...
if [[ "$monitor_count" == 1 ]]; then
  ...
  monitor_1=$(hyprctl monitors -j | jq -r 'sort_by(.x) | .[0].name')
  monitor_2=$(hyprctl monitors -j | jq -r 'sort_by(.x) | .[1].name')
    for ((i = 1; i <= 5; i++)); do
        echo "Workspace $i on $monitor_1"
        hyprctl keyword wsbind $i,$monitor_1 > /dev/null
        hyprctl dispatch moveworkspacetomonitor $i $monitor_1 > /dev/null
    ...
    monitor_1=$(hyprctl monitors -j | jq -r 'sort_by(.x) | .[0].name')
    monitor_2=$(hyprctl monitors -j | jq -r 'sort_by(.x) | .[1].name')
    monitor_3=$(hyprctl monitors -j | jq -r 'sort_by(.x) | .[2].name')
    for ((i = 1; i <= 3; i++)); do
        echo "Workspace $i on $monitor_1"
        hyprctl keyword wsbind $i,$monitor_1 > /dev/null
        hyprctl dispatch moveworkspacetomonitor $i $monitor_1 > /dev/null
